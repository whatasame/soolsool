...
class ReceiptAcceptanceTest extends AcceptanceTest {
    ...
    void setUpData() {
        ...
        final Long 새로_Id = RestLiquorFixture.술_등록_새로_판매중(최민족_토큰);
        final Long 얼음딸기주_Id = RestLiquorFixture.술_등록_과일주_전라북도_얼음딸기주_우영미_판매중(최민족_토큰);
    ...
    void createReceiptSuccess() {
        ...
        // when
        final ExtractableResponse<Response> createReceiptResponse = RestAssured.given()
                .log()
                .all()
    ...
    void findReceiptSuccess() {
        ...
        // when
        final ExtractableResponse<Response> detailReceiptResponse = RestAssured.given()
                .log()
                .all()
                .contentType(APPLICATION_JSON_VALUE)
                .header(AUTHORIZATION, BEARER + 김배달_토큰)
                .when()
                .get("/api/receipts/{receiptId}", 주문서_Id)
                .then()
                .log()
                .all()
                .extract();
        ...
        assertThat(detailReceiptResponse
                        .body()
                        .as(new TypeRef<ApiResponse<ReceiptDetailResponse>>() {})
                        .getMessage())
