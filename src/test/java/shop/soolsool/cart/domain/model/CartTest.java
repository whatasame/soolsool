...
class CartTest {
    ...
    @BeforeEach
    void setUpLiquor() {
        final LiquorBrew sojuBrew = new LiquorBrew(LiquorBrewType.SOJU);
        final LiquorBrew etcBrew = new LiquorBrew(LiquorBrewType.ETC);
        final LiquorRegion gyeongSangNamDoRegion = new LiquorRegion(LiquorRegionType.GYEONGSANGNAM_DO);
        final LiquorStatus onSaleStatus = new LiquorStatus(LiquorStatusType.ON_SALE);
        final LiquorStatus stoppedStatus = new LiquorStatus(LiquorStatusType.STOPPED);

        soju = new Liquor(
                1L, sojuBrew, gyeongSangNamDoRegion, onSaleStatus, "안동 소주", "12000", "안동", "/soju.jpg", 21.7, 400);
        beer = new Liquor(2L, etcBrew, gyeongSangNamDoRegion, stoppedStatus, "맥주", "5000", "OB", "/beer.jpg", 5.7, 500);
    }
    ...
    void createCart() {
        // given
        final List<CartItem> cartItems = List.of(new CartItem(1L, soju, 1), new CartItem(1L, beer, 1));
    ...
    void sameMember() {
        // given
        final List<CartItem> cartItems = List.of(new CartItem(1L, soju, 1), new CartItem(2L, beer, 1));
    ...
    void exceedMaxSize() {
        ...
        final Cart cart = new Cart(1L, cartItems);

        final CartItem newCartItem = new CartItem(1L, beer, 1);
    ...
    void duplicate() {
        // given
        final CartItem cartItem = new CartItem(1L, soju, 1);
        final CartItem sameCartItem = new CartItem(1L, soju, 1);

        final List<CartItem> cartItems = new ArrayList<>(List.of(cartItem));
    ...
    void stoppedLiquor() {
        // given
        final Cart cart = new Cart(1L, List.of());

        final Liquor stoppedLiquor = Liquor.builder()
        ...
        final CartItem cartItem = new CartItem(1L, stoppedLiquor, 1);
