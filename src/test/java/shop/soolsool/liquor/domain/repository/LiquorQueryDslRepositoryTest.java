...
class LiquorQueryDslRepositoryTest {
    ...
    void cursorFirstTestByLatest() {
        // given
        final LiquorBrew brew = null;
        final LiquorRegion region = null;
        final LiquorStatus status = null;
        final String brand = null;
        // when
        final List<Liquor> liquors = liquorQueryDslRepository.getList(
                new LiquorSearchCondition(region, brew, status, brand), Pageable.ofSize(2), null);
    ...
    void cursorClickCountNullTestByLatest() {
        ...
        // when
        final List<Liquor> 커서첫번째 = liquorQueryDslRepository.getList(
                new LiquorSearchCondition(region, brew, status, brand), Pageable.ofSize(1), null);
        final Long 커서첫번째_ID = 커서첫번째.get(커서첫번째.size() - 1).getId();

        final List<Liquor> 커서두번째 = liquorQueryDslRepository.getList(
                new LiquorSearchCondition(region, brew, status, brand), Pageable.ofSize(1), 커서첫번째_ID);
        final Long 커서두번째_ID = 커서두번째.get(커서두번째.size() - 1).getId();

        final List<Liquor> 커서세번째 = liquorQueryDslRepository.getList(
                new LiquorSearchCondition(region, brew, status, brand), Pageable.ofSize(1), 커서두번째_ID);
        final Long 커서세번째_ID = 커서세번째.get(커서세번째.size() - 1).getId();
    ...
    void cursorFirstTestByClickAndLatest() {
        ...
        final LiquorStatus status = null;
        final String brand = null;
        final Long liquorId = null;
        final Long clickCount = null;

        // when
        final List<LiquorClickElementDto> 커서첫번째 = liquorQueryDslRepository.getListByClick(
                new LiquorSearchCondition(region, brew, status, brand), Pageable.ofSize(1), liquorId, clickCount);
    ...
    void cursorClickCountNullTestByClickAndLatest() {
        ...
        final LiquorStatus status = null;
        final String brand = null;
        final Long liquorId = 3L;
        final Long clickCount = null;

        // when
        final List<LiquorClickElementDto> 커서첫번째 = liquorQueryDslRepository.getListByClick(
                new LiquorSearchCondition(region, brew, status, brand), Pageable.ofSize(1), liquorId, clickCount);
        final Long 커서첫번째_liquorId = 커서첫번째.get(커서첫번째.size() - 1).getId();
    ...
    void cursorLiquorIdClickCCountNotNullTestByClickAndLatest() {
        ...
        final String brand = null;
        final Long liquorId = 3L;
        final Long clickCount = 100L;

        // when
        final List<LiquorClickElementDto> 커서첫번째 = liquorQueryDslRepository.getListByClick(
                new LiquorSearchCondition(region, brew, status, brand), Pageable.ofSize(1), liquorId, clickCount);
        final Long 커서첫번째_liquorId = 커서첫번째.get(커서첫번째.size() - 1).getClickCount();
        final Long 커서첫번째_clickCount = 커서첫번째.get(커서첫번째.size() - 1).getClickCount();

        final List<LiquorClickElementDto> 커서두번째 = liquorQueryDslRepository.getListByClick(
                new LiquorSearchCondition(region, brew, status, brand),
                Pageable.ofSize(1),
                커서첫번째_liquorId,
                커서첫번째_clickCount);
        final Long 커서두번째_clickCount = 커서두번째.get(커서두번째.size() - 1).getClickCount();
