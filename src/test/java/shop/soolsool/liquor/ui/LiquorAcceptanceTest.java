...
import static shop.soolsool.liquor.code.LiquorResultCode.LIQUOR_CREATED;
import static shop.soolsool.liquor.code.LiquorResultCode.LIQUOR_DELETED;
import static shop.soolsool.liquor.code.LiquorResultCode.LIQUOR_UPDATED;
...
class LiquorAcceptanceTest extends AcceptanceTest {
    ...
    void saveLiquor() {
        ...
        final LiquorSaveRequest liquorSaveRequest =
                new LiquorSaveRequest("SOJU", "GYEONGGI_DO", "ON_SALE", "새로", "3000", "브랜드", "/soju-url", 12.0, 300);
        ...
        assertThat(response.body().as(new TypeRef<ApiResponse<Void>>() {}).getMessage())
    ...
    void modifyLiquorTest() {
        ...
        final ExtractableResponse<Response> response = RestAssured.given()
                ...
                .when()
                .put("/api/liquors/{liquorId}", 새로_Id)
                .then()
                .log()
                .all()
                .extract();
        ...
        assertThat(response.body().as(new TypeRef<ApiResponse<Void>>() {}).getMessage())
                .isEqualTo(LIQUOR_UPDATED.getMessage());
    }
    ...
    void removeLiquorTest() {
        ...
        final ExtractableResponse<Response> response = RestAssured.given()
                ...
                .header(HttpHeaders.AUTHORIZATION, BEARER + accessToken)
                .when()
                .delete("/api/liquors/{liquorId}", 새로_Id)
                .then()
                .log()
                .all()
                .extract();
        ...
        assertThat(response.body().as(new TypeRef<ApiResponse<Void>>() {}).getMessage())
                .isEqualTo(LIQUOR_DELETED.getMessage());
    }
    ...
    void liquorDetail() {
        ...
        // when
        final LiquorDetailResponse liquorDetailResponse = RestAssured.given()
                .log()
                .all()
                .contentType(APPLICATION_JSON_VALUE)
                .accept(APPLICATION_JSON_VALUE)
                .when()
                .get("/api/liquors/{liquorId}", 새로_Id)
                .then()
                .log()
                .all()
                .extract()
    ...
    void liquorPurchasedTogether() {
        // given
        final String vendorAccessToken = RestAuthFixture.로그인_최민족_판매자();
        final Long 새로_Id = RestLiquorFixture.술_등록_새로_판매중(vendorAccessToken);
        RestLiquorStockFixture.술_재고_등록(vendorAccessToken, 새로_Id, 100);
        final Long 얼음딸기주_Id = RestLiquorFixture.술_등록_과일주_전라북도_얼음딸기주_우영미_판매중(vendorAccessToken);
        RestLiquorStockFixture.술_재고_등록(vendorAccessToken, 얼음딸기주_Id, 100);

        final String customerAccessToken = RestAuthFixture.로그인_김배달_구매자();
        RestCartFixture.장바구니_상품_추가(customerAccessToken, 새로_Id, 1);
        RestCartFixture.장바구니_상품_추가(customerAccessToken, 얼음딸기주_Id, 1);
        final Long 주문서_Id = RestReceiptFixture.주문서_생성(customerAccessToken);
        RestPayFixture.결제_준비(customerAccessToken, 주문서_Id);
        RestPayFixture.결제_성공(customerAccessToken, 주문서_Id);

        // when
        final List<LiquorElementResponse> responses = RestAssured.given()
                .log()
                .all()
                .contentType(APPLICATION_JSON_VALUE)
                .accept(APPLICATION_JSON_VALUE)
                .when()
                .get("/api/liquors/{liquorId}/related", 새로_Id)
                .then()
                .log()
                .all()
                .extract()
                .jsonPath()
                .getList("data", LiquorElementResponse.class);

        final List<Long> relatedLiquorIds =
                responses.stream().map(LiquorElementResponse::getId).collect(Collectors.toList());
    ...
    void liquorList() {
        ...
        // when
        final List<LiquorElementResponse> liquors = RestAssured.given()
                .log()
                .all()
                .contentType(APPLICATION_JSON_VALUE)
                .accept(APPLICATION_JSON_VALUE)
                ...
                .extract()
                .jsonPath()
                .getObject("data", PageLiquorResponse.class)
                .getLiquors();
