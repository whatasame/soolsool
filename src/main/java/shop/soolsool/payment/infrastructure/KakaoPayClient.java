...
import shop.soolsool.payment.domain.repository.KakaoPayReceiptRepository;
import shop.soolsool.payment.infrastructure.response.KakaoPayApproveResponse;
import shop.soolsool.payment.infrastructure.response.KakaoPayReadyResponse;
...
public class KakaoPayClient implements PayClient {
    ...
    public PayReadyResponse ready(final Receipt receipt) {
        final HttpEntity<MultiValueMap<String, String>> body =
                new HttpEntity<>(getKakaoPayReadyBody(receipt), getKakaoPayHeaders());

        final KakaoPayReadyResponse kakaoPayReadyResponse = readyForKakaoPay(body);

        kakaoPayReceiptRepository.save(KakaoPayReceipt.of(receipt.getId(), kakaoPayReadyResponse.getTid()));
    ...
    private KakaoPayReadyResponse readyForKakaoPay(final HttpEntity<MultiValueMap<String, String>> body) {
        final KakaoPayReadyResponse kakaoPayReadyResponse =
                restTemplate.postForObject(URI.create(HOST + READY_ENDPOINT), body, KakaoPayReadyResponse.class);
    ...
    public PayApproveResponse payApprove(final Object... args) {
        ...
        final HttpEntity<MultiValueMap<String, String>> body =
                new HttpEntity<>(getKakaoPayApproveBody(receipt, pgToken, kakaoPayReceipt), getKakaoPayHeaders());
    ...
    private KakaoPayApproveResponse approveForKakaoPay(final HttpEntity<MultiValueMap<String, String>> body) {
        final KakaoPayApproveResponse kakaoPayApproveResponse =
                restTemplate.postForObject(URI.create(HOST + APPROVE_ENDPOINT), body, KakaoPayApproveResponse.class);
    ...
    private MultiValueMap<String, String> getKakaoPayApproveBody(
            final Receipt receipt, final String pgToken, final KakaoPayReceipt kakaoPayReceipt) {
        final MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
        params.add("cid", CID);
