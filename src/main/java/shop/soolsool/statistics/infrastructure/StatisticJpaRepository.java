...
import shop.soolsool.statistics.domain.model.Statistic;
import shop.soolsool.statistics.domain.model.StatisticId;
...
public interface StatisticJpaRepository extends JpaRepository<Statistic, StatisticId> {
    ...
    @Modifying
    @Query(
            value = "INSERT INTO statistics (liquor_id, statistic_year, statistic_month, statistic_week, "
            ...
            nativeQuery = true)
    void updateStatisticsSales(final LocalDate startDate, final LocalDate endDate);

    @Modifying
    @Query(
            value = "INSERT INTO statistics (statistic_year, statistic_month, statistic_week, "
                    ...
                    + "                        click = (lc.click - s.sum_click)",
            nativeQuery = true)
    void updateStatisticsCtr(final LocalDate date);

    @Query(
            value = "SELECT l.id as liquorId, "
                    ...
                    + "    LIMIT 5 "
                    + ") s "
                    + "JOIN liquors l ON s.liquor_id = l.id ",
            nativeQuery = true)
    ...
    @Query(
            value = "select liquor_id as liquorId, sum(sale_price) as liquorValue "
                    + "from statistics "
                    + "group by liquor_id "
                    + "order by sum(sale_price) desc "
                    + "limit 5",
    ...
    @Query(
            value = "select liquor_id as liquorId, sum(sale_quantity) as liquorValue "
                    + "from statistics "
                    + "group by liquor_id "
                    + "order by sum(sale_quantity) desc "
                    + "limit 5",
