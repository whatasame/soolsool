...
import shop.soolsool.liquor.code.LiquorStockErrorCode;
...
public class LiquorStocks {
    ...
    public void decreaseStock(int quantity) {
        ...
        for (final LiquorStock liquorStock : liquorStocks) {
            if (quantity == 0) {
                break;
            }
            final int target = Math.min(liquorStock.getStock(), quantity);
            quantity -= target;
    ...
    private void validateEnoughStocks(final int quantity) {
        final int totalStock =
                liquorStocks.stream().mapToInt(LiquorStock::getStock).sum();
    ...
    public List<LiquorStock> getOutOfStocks() {
        return this.liquorStocks.stream().filter(LiquorStock::isOutOfStock).collect(Collectors.toList());
    }
}
