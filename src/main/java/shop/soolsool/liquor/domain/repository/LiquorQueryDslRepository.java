...
import static shop.soolsool.liquor.domain.model.QLiquor.liquor;
import static shop.soolsool.liquor.domain.model.QLiquorCtr.liquorCtr;
...
public class LiquorQueryDslRepository {
    ...
    public List<Liquor> getList(final LiquorSearchCondition condition, final Pageable pageable, final Long liquorId) {
        return queryFactory
                .select(liquor)
                .from(liquor)
                .where(
    ...
    public List<LiquorClickElementDto> getListByClick(
        ...
        return queryFactory
                .select(Projections.constructor(LiquorClickElementDto.class, liquor, liquorCtr.click))
                .from(liquor)
                .join(liquorCtr)
                .on(liquor.id.eq(liquorCtr.liquorId))
                .where(
                        eqRegion(condition.getLiquorRegion()),
                        eqBrew(condition.getLiquorBrew()),
                        eqStatus(condition.getLiquorStatus()),
    ...
    private BooleanExpression cursorId(final Long liquorId, final Long click) {
        ...
        return liquorCtr
                ...
                .count
                .lt(click)
                .or(liquorCtr.click.count.eq(click).and(liquor.id.lt(liquorId)));
    }
}
