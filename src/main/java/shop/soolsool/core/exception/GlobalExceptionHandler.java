...
import shop.soolsool.core.code.DefaultErrorCode;
...
public class GlobalExceptionHandler {
    ...
    protected ResponseEntity<ErrorResponse> handleSoolSoolException(final SoolSoolException e) {
        ...
        if (isInternalServerError(errorCode)) {
            log.error(
                    "!!! SoolSoolException INTERNAL SERVER ERROR !!! | code : {} | message : {}",
                    errorCode.getCode(),
                    e.getMessage());
        } else {
            log.warn("SoolSoolException | code : {} | message : {}", errorCode.getCode(), e.getMessage());
        }
    ...
    protected ResponseEntity<ErrorResponse> handleValidationException(final MethodArgumentNotValidException exception) {
        ...
        log.warn("Validation Error | code : {} | message : {}", errorCode.getCode(), exception.getMessage());
    ...
    protected ResponseEntity<ErrorResponse> handleException(final Exception e) {
        ...
        log.error(
                "!!! 예상치 못한 예외가 발생했습니다 !!! | code : {} | message : {} | log : {}",
                errorCode.getCode(),
                e.getMessage(),
                e.getStackTrace());
